#include <unistd.h>
#include <stdio.h>

void	print_address(unsigned int num, int index)
{
	const char	*hex = "0123456789abcdefgh";
	int			buffer[16];
	
	while (index < 16)
	{
		buffer[index] = 0;
		index++;
	}
	index = 0;
	while (num > 15)
	{
		buffer[index] = num % 16;
		num /= 16;
		index++;
	}
	index = 15;
	while (index >= 0)
	{
		write (1, &hex[buffer[index]], 1);
		index--;
	}
	write(1, ": ", 2);
}

void	print_char_hex(void *addr, unsigned char *cur_addr, int size)
{
	int		i;
	char	*hex = "0123456789abcdef";

	i = 0;
	while (i < 16)
	{
		if ((void *)(cur_addr + i) < addr + size)
		{
			write(1, &hex[cur_addr[i] / 16], 1);
			write(1, &hex[cur_addr[i] % 16], 1);
		}
		else
			write(1, "  ", 2);
		if (i % 2 == 1)
			write(1, " ", 1);
		i++;
	}
}

void	print_str(void *addr, char *cur_addr, int size)
{
	int	i;

	i = 0;
	while (i < 16 && ((void *)(cur_addr + i) < addr + size))
	{
		if (cur_addr[i] >= ' ' && cur_addr[i] != 127)
			write(1, &cur_addr[i], 1);
		else
			write(1, ".", 1);
		i++;
	}
	write(1, "\n", 1);
}

void	*ft_print_memory(void *addr, unsigned int size)
{
	int	i;
	char	*cur_addr;

	i = 0;
	cur_addr = (char *)addr;
	while ((void *)cur_addr < (addr + size))
	{
		print_address((unsigned int)cur_addr, 0);
		print_char_hex(addr, (unsigned char *)cur_addr, size);
		print_str(addr, cur_addr, size);
		cur_addr += 16;
	}
	return (addr);
}

int main(void)
{
	char mem[] = {
		 };
	ft_print_memory(mem, 1500);
	// char	*string;

	// string = "Bonjour les aminches\t\n\tc  est fou.tout.ce qu on peut faire avec...print_memory....lol.lol. ";
	// ft_print_memory(string, 1500);
}